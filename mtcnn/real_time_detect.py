...
def drawBoxes(im, boxes):
    for rectangle in boxes:
        ...
        im=misc.imresize(cropped, (160,160), interp='bilinear')
        #im=cv2.resize(cropped,(96,96),interpolation=cv2.INTER_LINEAR)
...
def detect_face(img, minsize, PNet, RNet, ONet, threshold, fastresize, factor):
    ...
    for scale in scales:
        ...
        if fastresize:
            im_data = (img-127.5)/128 # [0,255] -> [-1,1]
            im_data = cv2.resize(im_data, (ws,hs)) # default is bilinear
        else:
            im_data = cv2.resize(img, (ws,hs)) # default is bilinear
            im_data = (im_data-127.5)/128 # [0,255] -> [-1,1]
    ...
    if numbox > 0:
        ...
        #print(tempimg.shape)
        #print(tempimg[0,0,0,:])
        tempimg = (tempimg-127.5)/128 # done in imResample function wrapped by python
        ...
        if numbox > 0:
            ...
            tempimg = (tempimg-127.5)/128 # [0,255] -> [-1,1]
...
def mtcnn(img):
    ...
    img_matlab[:,:,0] = tmp
    '''
    caffe_model_path = sys.path[-1]+"./model"
    caffe.set_device(0)
    caffe.set_mode_cpu()
    global PNet
    PNet = caffe.Net(caffe_model_path+"/det1.prototxt", caffe_model_path+"/det1.caffemodel", caffe.TEST)
    ...
    '''
    faces=[]
    bounding_boxes, points = detect_face(img_matlab, minsize, PNet, RNet, ONet, threshold, False, factor)
    for bb in bounding_boxes:
        face = Face()
        face.container_image = img_matlab
        face.bounding_box = np.zeros(4, dtype=np.int32)

        img_size = np.asarray(img_matlab.shape)[0:2]
        ...
        cropped = img_matlab[face.bounding_box[1]:face.bounding_box[3], face.bounding_box[0]:face.bounding_box[2], :]
